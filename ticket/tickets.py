...
class TrainsCollection:
	...
	header = '车次 车站 时间 历时 商务特等 一等 二等 高级软卧 软卧 动卧 硬卧 硬座 无座 备注'.split()
	...
	def trains(self):
		...
		for raw_train in self.available_trains:
			...
			#test
			for k,v in enumerate(raw_train):
				print(k,v)
			...
			if not self.options or inital in self.options:
				...
				raw_train[32] or '--', # 商务特等
				raw_train[31] or '--', # 一等座
				raw_train[30] or '--', # 二等座
				raw_train[21] or '--', # 高级软卧
				raw_train[23] or '--', # 软卧
				raw_train[33] or '--', # 动卧
				raw_train[28] or '--', # 硬卧
				raw_train[29] or '--', # 硬座
				raw_train[26] or '--', # 无座
				raw_train[1].replace('<br/>','\n') or '--'  #备注 
				]
				yield train
